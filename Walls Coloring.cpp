...
either pink, black, or yellow. The cost associated with coloring each wall with a particular color is represented by a 2D array colors of size N*3 , where 
colors[i][0], colors[i][1], and colors[i][2] is the cost of painting ith wall with colors pink, black, and yellow respectively.
...
public:
    ...
    for(int i=1;i<N;i++){
        ...
        curr[0]=min(prev[1],prev[2])+colors[i][0];
        curr[1]=min(prev[0],prev[2])+colors[i][1];

        curr[2]=min(prev[1],prev[0])+colors[i][2];
        prev=curr;
